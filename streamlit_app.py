...
def predict_species(features):
    # URL actualizada y gestión de errores mejorada
    api_url = "http://localhost:8000/predict"
    try:
        # Aumentar el timeout para dar más tiempo a la API para responder
        ...
        st.error(
            "Error de conexión: No se pudo conectar a la API. Asegúrate de que la API está en ejecución en localhost:8000.")
        return None
...
with col2:
    ...
    try:
        api_status = requests.get("http://localhost:8000/health", timeout=2)
        if api_status.status_code == 200:
            st.success("✅ API conectada y funcionando")
        else:
            st.error("❌ API no responde correctamente")
    except:
        st.error("❌ API no disponible - Asegúrate de que está en ejecución")
